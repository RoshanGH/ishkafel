...
void main() {
  group('把没有区分度的标签剔出检索键', () {
    ...
    test('只有一个标签时不动它——没有别的标签可比，谈不上宽泛', () {
      final plan = narrowTagQuery(hits: [hit('实拍', 1, 5437)]);
    ...
    test('宽泛与否看断层，不写死数字', () {
      // 1000 / 900 只差 1.1 倍，900 / 30 差 30 倍——坎在后面
      final plan = narrowTagQuery(hits: [
        hit('宽', 1, 1000),
        hit('也宽', 2, 900),
        hit('窄', 3, 30),
      ...
      expect(plan.tagIds, [3]);
      expect(plan.droppedBroad, ['宽', '也宽']);
    });
