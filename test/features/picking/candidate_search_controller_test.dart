...
void main() {
  group('检索结果装载', () {
    test('成功：先出候选卡（规格标为探测中），再逐条填规格', () async {
      ...
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async => _ok(_searchJson(2)), binary: 'miaoa')),
    ...
    test('规格探测并发有上限，不会一次把二十条全打出去', () async {
      ...
      var peak = 0;
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async => _ok(_searchJson(20)), binary: 'miaoa')),
        probe: CandidateProbe(run: (_, _) async {
          inFlight++;
    ...
    test('0 条结果是正常状态（引导语由页面按检索方式给），不是失败', () async {
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async => _ok(_searchJson(0)), binary: 'miaoa')),
    ...
    test('总数与被跳过的畸形条目如实带出（不静默丢弃）', () async {
      ...
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async => _ok(json), binary: 'miaoa')),
        probe: CandidateProbe(run: (_, _) async => _ok(_probeStdout)),
      );
      await controller.searchByDescription('喷雾');
  ...
  group('失败提示：服务层的中文原样透出', () {
    test('401 直接给出可照做的中文，不包一层「未知错误」', () async {
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async => ProcessResult(1, 1, '', 'HTTP 401 unauthorized'), binary: 'miaoa')),
        probe: CandidateProbe(run: (_, _) async => _ok(_probeStdout)),
      );
      await controller.searchByDescription('喷雾');
      expect(controller.status, CandidateSearchStatus.failed);
      expect(controller.failureMessage, contains('重新登录'));
      expect(controller.failureMessage, contains('miaoa auth login'));
      expect(controller.entries, isEmpty);
    });

    test('未选标签就检索：服务层的中文提示照样透出，不崩', () async {
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async => _ok(_searchJson(1)), binary: 'miaoa')),
        probe: CandidateProbe(run: (_, _) async => _ok(_probeStdout)),
      );
    ...
    test('重试成功后失败提示要清掉', () async {
      var fail = true;
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async =>
            fail ? ProcessResult(1, 1, '', 'timeout') : _ok(_searchJson(1)), binary: 'miaoa')),
        probe: CandidateProbe(run: (_, _) async => _ok(_probeStdout)),
      );
      await controller.searchByDescription('喷雾');
  ...
  group('竞态：用户手快切换检索方式', () {
    test('过期的检索结果不覆盖新的（否则界面会闪回上一次的候选）', () async {
      ...
      var call = 0;
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, args) async {
          if (call++ == 0) {
    ...
    test('过期检索的规格探测也不写进新结果', () async {
      ...
      final controller = CandidateSearchController(
        service: MiaoaContentService(gateway: MiaoaGateway(run: (_, _) async =>
            _ok(_searchJson(searchCall++ == 0 ? 1 : 1)), binary: 'miaoa')),
        probe: CandidateProbe(run: (_, _) async {
