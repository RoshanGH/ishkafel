...
void main() {
  ...
  Future<ProcessResult> runScript(
      {required String newApp, required String target}) async {
    final f = File('${work.path}/replace.sh');
    f.writeAsStringSync(AppUpdater().replaceScript(
      newApp: newApp,
      targetApp: target,
  ...
  test('换上去了：新的就位，旧的清干净', () async {
    ...
    expect(File('${target.path}/marker').readAsStringSync(), '新',
        reason: '换上去了没有，看的是内容，不是退出码');
    expect(r.exitCode, 0,
        reason: '换好了就算成功——打开失败不该让退出码看起来像「升级失败」');
  ...
  test('搬不过去：旧的必须回到原位，一个字节都不能少', () async {
    ...
    final r = await runScript(
        newApp: '${work.path}/根本不存在.app', target: target.path);
  ...
  test('等旧进程退出：pid 还活着就不动手', () async {
    ...
    await Process.run('chmod', ['+x', f.path]);
    final proc = await Process.start('/bin/sh', [f.path]);
