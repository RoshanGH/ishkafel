...
void main() {
  group('帧对齐（错开一帧不会报错，只会让结果悄悄失真）', () {
    test('第 j 条记录对应第 j+1 帧的画面变化', () {
      ...
      final signals =
          buildFrameSignals(sceneMetadata: meta, thumbBytes: thumbs, side: _side);

      expect(signals, hasLength(2));
      expect(signals[0].histDistance, 0,
          reason: '第 1 帧相对第 0 帧没有变化（黑→黑）');
      expect(signals[1].histDistance, closeTo(2, 0.01),
          reason: '第 2 帧相对第 1 帧是黑→白。若把记录对到第 j 帧而不是 '
  ...
  group('两份产物长度对不上时不越界', () {
    ...
    test('metadata 为空时返回空', () {
      ...
      expect(
          buildFrameSignals(
              sceneMetadata: '', thumbBytes: thumbs, side: _side),
          isEmpty);
    });
  });
  ...
  group('ffmpeg 参数', () {
    test('scene 用 gt(scene,0)：每帧都出记录，阈值判定留给 Dart 侧', () {
      ...
      expect(args.join(' '), contains("gt(scene,0)"),
    ...
    test('缩略图带 -y，重跑分析不会卡在覆盖确认上', () {
      ...
      expect(args.first, '-y',
          reason: '本项目踩过这个坑：-y 放在输入之后不生效，ffmpeg 会等在'
