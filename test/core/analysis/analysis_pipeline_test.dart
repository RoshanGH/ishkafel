...
void main() {
  ...
  test('analyze 产出两层结构并落库，状态转 awaitingCut', () async {
    ...
    final result = await makePipeline(repo).analyze(task, by: ActorKind.agent, actor: 'Agent',);
  ...
  test('videoInfo 缺失抛 StateError 且不落库变更', () async {
    ...
    await expectLater(
        makePipeline(repo).analyze(noInfo, by: ActorKind.agent, actor: 'Agent',), throwsA(isA<StateError>()));
  ...
  group('口播与背景音分离', () {
    ...
    test('没装分离工具时照常分析', () async {
      ...
      final done = await makePipeline(repo).analyze(task, by: ActorKind.agent, actor: 'Agent',);
  ...
  group('人声轨不跨任务共用', () {
    ...
    test('删掉先分析那条任务的产物后，另一条任务仍有自己能用的人声轨', () async {
      ...
      await repo.save(a);
      final doneA =
          await makePipeline(repo, separator: fakeSeparator()).analyze(a, by: ActorKind.agent, actor: 'Agent',);
      ...
      await repo.save(b);
      final doneB =
          await makePipeline(repo, separator: fakeSeparator()).analyze(b, by: ActorKind.agent, actor: 'Agent',);
    ...
    test('单独重新分离：只补这条任务自己的人声轨，不重跑分析', () async {
      ...
      final a = makeTask(id: '任务A', sourcePath: source.path);
      await repo.save(a);
      final done = await pipeline.analyze(a, by: ActorKind.agent, actor: 'Agent',);
      // 产物被清掉（别人删任务、或磁盘清理）
      Directory('${tempDir.path}/work/stems/任务A').deleteSync(recursive: true);
