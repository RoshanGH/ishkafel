...
void main() {
  test('把标签组内的标签名取成受控词表', () async {
    final source = MiaoaTagVocabularySource(MiaoaTagService(
        run: (_, _) async => ProcessResult(1, 0, _tagListJson, '')));
  ...
  test('按传入的组 id 查（不同任务查不同组）', () async {
    final asked = <String>[];
    final source = MiaoaTagVocabularySource(MiaoaTagService(run: (_, args) async {
      asked.add(args.join(' '));
  ...
  test('CLI 失败原样抛出，由调用方决定降级策略（不静默吞成空词表）', () async {
    final source = MiaoaTagVocabularySource(MiaoaTagService(
        run: (_, _) async => ProcessResult(1, 1, '', '401 Unauthorized')));
