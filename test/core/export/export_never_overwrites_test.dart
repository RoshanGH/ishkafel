...
Future<ProcessResult> _ffmpeg(String bin, List<String> args) async {
  File(args.last).writeAsStringSync('这一批 ${DateTime.now().microsecondsSinceEpoch}');
...
void main() {
  test('导第二批到同一个目录，上一批的成片还在——不能被同名顶掉', () async {
    ...
    expect(
      ...
      4,
      reason: '两批各两条，四个文件都该在',
    );
    expect(
      second.map((p) => p.split('/').last),
      ['变体1(2).mp4', '变体2(2).mp4'],
    );
  });
  ...
  test('同一批里两条方案重名，后一条也不覆盖前一条', () async {
    ...
    final combos = ExportPlanner.enumerate(
      units: _units(),
      replacements: [UnitReplacement.whole(const [11, 12])],
    )
        .map((c) => ExportCombination(
            index: c.index, segments: c.segments, name: '居家写实线'))
        .toList();
    ...
    expect(results.map((r) => r.path!.split('/').last),
        ['居家写实线.mp4', '居家写实线(2).mp4']);
