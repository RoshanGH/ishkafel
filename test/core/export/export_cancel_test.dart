...
void main() {
  ...
  test('导到一半按停止：已导完的全留着，剩下的标「已停止」不是「失败」', () async {
    ...
    expect(rest.every((r) => r.failure == null), isTrue,
  ...
  test('正在导的那条不许在输出目录里留半成品', () async {
    ...
    final files = b.out
        ...
        .toList();
    expect(files, ['变体1.mp4'],
        reason: '输出目录里只该有导完的那一条——半成品看起来和成片一模一样，'
            '留着迟早被当成能交付的东西发出去');
  ...
  test('开了停止之后一步都不许再跑——按下就是按下', () async {
    ...
    expect(callsAfterCancel, 0,
        reason: '检查点在拉子进程之前，按下之后不该再起新的 ffmpeg');
  });
}
