...
///    而且不能只是 `unfocus()`：焦点落空之后 Flutter 不知道该把按键送到哪个
///    `Shortcuts` 上，空格和方向键会**一起哑掉**。得交回 [KeyboardHome]。
///
...
/// 弹层一关焦点就跟着没了。
void main() {
  test('页面上的输入框都要把焦点交回 KeyboardHome', () {
    const fields = {
      // 工作台：单元台词
    ...
    for (final entry in fields.entries) {
      ...
      final released = 'KeyboardHome.take'.allMatches(src).length;
    ...
    expect(offenders, isEmpty,
        reason: '这些输入框打完字点到别处，焦点没人接住，空格就再也回不到播放器：\n'
            '${offenders.join('\n')}');
  });
}
