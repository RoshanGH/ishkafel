...
void main() {
  test('没有人绕过 ScriptDoc.refVideoOf 自己拼回落', () {
    ...
    for (final dir in ['lib/cli', 'lib/features', 'lib/core']) {
      for (final f in Directory(dir)
        ...
        if (f.path.endsWith('core/script/script_doc.dart')) continue;
    ...
    expect(offenders, isEmpty,
        reason: '这些地方在自己拼「行级 ?? 文档级」：${offenders.join('、')}。'
  ...
  test('要复刻的那三条命令都从文档上取参考片，不只看行级', () {
    ...
    expect(src.contains('refVideoOf'), isTrue,
        reason: 'tag-ref / frames / shots 是参考片复刻的前三步，'
