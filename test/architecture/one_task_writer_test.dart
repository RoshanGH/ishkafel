...
void main() {
  ...
  /// 白名单三：**一次性的存档迁移写回**。
  ///
  /// 这一类和「建新任务」不同——它确实在改已存在的任务——但三条特征凑齐了
  /// 就不该走 `TaskMutation`：
  ///
  /// 1. **必须保留原本的 `updatedAt`**。`apply` 统一把它推到当下，而这一笔
  ///    写回并不是「这条任务上发生了一件事」。推了会出两种事故：任务列表按
  ///    `updatedAt` 倒序，升级后第一次打开**所有老任务一起跳到最前面**
  ///    （人看到的是「我的任务全乱了」）；`TaskMutation` 自己又拿
  ///    `updatedAt` 当版本令牌，推一下等于凭空制造一次「被抢写」的误判。
  /// 2. **正确性不依赖这次写回**。补出来的值是确定性推导的，写不成下次读
  ///    照样推得出同一个，所以它只是省掉每次读都重算的优化。
  /// 3. **日志里不该有它**。没有用户意义上的事发生，记一笔只会稀释日志。
  ///
  /// **先例是 `file_task_repository.dart:findById` 里的单元身份（uid）补发**
  /// ——那一处的长注释把这三条讲透了（尤其「这次写回保留的是原本读到的
  /// `updatedAt`，不是 `DateTime.now()`」那一段），它本身在白名单一里整份豁免。
  /// `ensureTaskSeqs` 补的「#12」短编号是同一类：`seq` 是后加的字段，老存档
  /// 没有，补的是个只给人念的显示号。
  ///
  /// 同样 narrow 到「文件 + 函数」——以后谁往 `task_seq.dart` 里加一处
  /// 「改已存在任务」的真写入，照样会被扫到。
