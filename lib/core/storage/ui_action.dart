...
enum UiAction {
  ...
  /// 提交替换方案。
  ///
  /// **为什么要能委派**：让人看着三条方案一条条落到时间线上，而且
  /// **人看到的和落盘的同源**——界面演一套、CLI 写一套是这个项目栽过
  /// 三次的那个毛病的更大版本。
  ///
  /// 它**不是必经之路**：界面不在这条任务上就零等待自己直写，界面在但
  /// 秒级没应就兜底自己直写，两条路落的是同一份 `_commitPlans`
  /// （见 `lib/cli/delegate.dart`）。
  ///
  /// 历史：这条委派一度只在「界面占着写锁」时才走得通——于是整条流水线里
  /// 最该让人看见的一步，恰恰因为「人在看」而做不了。锁 2026-09-18 删掉了，
  /// 委派留下来，因为它本来就是为了让人看见，不是为了绕开谁。
  plansApply('plans.apply', '提交替换方案'),

  /// 打开导出对话框，参数填好，**最后那一下由人点**。
  ///
  /// 和提交方案不一样的地方：导出跑几分钟、直接产出交付物、又花钱——
  /// 人在旁边时让他确认一下反而是对的。所以委派的是「打开、填好」，
  /// 不是「一路点到底」。
  ///
  /// 同样**不是必经之路**：界面不在这条任务上就根本不下这一单、自己导；
  /// 下了单而界面没接，命令会把单子收回来自己导——绝不返回失败。
  exportOpen('export.open', '打开导出'),

  /// 把界面从当前任务退回任务列表。
  ///
  /// **现在它只是一句「回列表看看」**：人开着哪一页都不会挡住 Agent 写那条
  /// 任务，所以用不用随你。想让人看着你干活，反过来用 `ui open` 把界面
  /// 带到现场。
  ///
  /// 历史（解释它为什么曾经是必需品）：`ui new-task` 建完任务界面就停在
  /// 那条任务上占着写锁，下一步非写它不可，于是「建完立刻干活」这条最自然
  /// 的路走不通，只能先把界面支开。那把锁 2026-09-18 删掉了。
