...
class ExportCommands {
  ...
  /// 「怎么把一层声音叠上去」这套规则的版本号。**改了 [_placeClip] 或
  /// [_sumTwo] 就要把它 +1。**
  ///
  /// 中间产物按内容指纹缓存，而指纹是用入参拼的（素材路径、位置、时长、倍率、
  /// 音量），规则本身不在里面。2026-09-11 改完滤镜链之后，盘上那些**按错规则
  /// 渲出来的** `mix_material_*.wav` 入参一个没变、键也一个没变，下次导出
  /// 照样命中——用户装了新版重新导出，听到的还是老毛病，而哪儿都不报错。
  ///
  /// v2 = 「先裁成自己那么长再摆到位置上」+「各层相加」那一版
  static const String mixRuleVersion = 'v2';
